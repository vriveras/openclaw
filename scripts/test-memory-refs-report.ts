...
async function main() {
  ...
  const runSuite = async (label: string, recursiveCfg?: RecursiveCfg) => {
    ...
    for (const tc of gt.cases) {
      ...
      const baseline = await search.execute("tc-baseline", { query, maxResults: gt.defaults.maxResults });
    ...
    suite.summary = {
      ...
      recursivePassRate: recPass.length ? recPass.reduce((a, b) => a + b, 0) / recPass.length : null,
      aggregates: {
        tokens: {
  ...
  if (args.sweep) {
    ...
    const withRec = candidates
      ...
      .map((c) => ({
        ...
        recLatencyP95: c.summary.aggregates.latencyMs.recursiveRefs?.p95 ?? Number.POSITIVE_INFINITY,
      }));
    ...
    report.sweep = {
      ...
      objective: "maximize passRate, then minimize recursiveRefs.meanTokens, then minimize recursiveRefs.p95Latency",
