...
async function main() {
  ...
  for (const query of queries) {
    ...
    // 2) Refs-first
    const refsOut = await searchRefs.execute("toolcall-2", { query, maxResults: 8, previewChars: 140 });
    ...
    for (const [i, r] of refs.slice(0, 3).entries()) {
      console.log(`  [${i + 1}] ${r.path}:${r.startLine}-${r.endLine} :: ${(r.preview ?? "").slice(0, 120)}`);
    }
  }
